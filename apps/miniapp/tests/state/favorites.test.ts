...
function requireBarrier(): () => () => void {
  const block = (favoritesState as Record<string, unknown>).blockFavoriteMutations;
  expect(block, "blockFavoriteMutations must be exported for destructive flows").toBeTypeOf("function");
...
describe("favorites state", () => {
  ...
  it("holds new favorite intents behind a global barrier without optimistic state until release", async () => {
    installWx();
    addFavorite.mockResolvedValueOnce(undefined);
    const blockFavoriteMutations = requireBarrier();
    const release = blockFavoriteMutations();

    const mutation = setFavorite("dish-barrier", true);
    await Promise.resolve();
    const idsWhileBlocked = getFavoriteState().ids;
    const pendingWhileBlocked = getFavoriteState().pending;
    const remoteCallsWhileBlocked = addFavorite.mock.calls.length;

    release();
    await mutation;

    expect(remoteCallsWhileBlocked).toBe(0);
    expect(idsWhileBlocked).toEqual(new Set());
    expect(pendingWhileBlocked).toEqual(new Set());
    expect(addFavorite).toHaveBeenCalledTimes(1);
    expect(getFavoriteState().ids).toEqual(new Set(["dish-barrier"]));
  });

  it("cancels a blocked favorite intent when favorites are cleared before release", async () => {
    installWx();
    addFavorite.mockResolvedValueOnce(undefined);
    const blockFavoriteMutations = requireBarrier();
    const release = blockFavoriteMutations();
