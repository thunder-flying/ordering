  ...
  it.each([
    {
      operation: "create",
      editTarget: null,
      expectedCalls: { create: 1, update: 0 },
    },
    {
      operation: "update",
      editTarget: {
      ...
      expectedCalls: { create: 0, update: 1 },
    },
  ] as const)(
    "keeps a newer draft and returns saved-stale when a $operation request succeeds",
    async ({ editTarget, expectedCalls }) => {
      let draft = makeDraft(editTarget);
      const response = deferred<SavedListDetailDto>();
      let createCalls = 0;
      let updateCalls = 0;
      let clearCalls = 0;
      const controller = createCurrentListSaveController(saveDependencies({
        ...
        update: async (_listId: string, _input: unknown) => {
          updateCalls += 1;
          return response.promise;
        },
        clearDraft: () => {
          clearCalls += 1;
          draft = { ...makeDraft(), name: "", items: [] };
        },
      }));

      const saving = controller.save();
      await Promise.resolve();
      expect({ create: createCalls, update: updateCalls }).toEqual(expectedCalls);
      ...
      await expect(saving).resolves.toEqual({ status: "saved-stale", list: savedList });
      expect(clearCalls).toBe(0);
