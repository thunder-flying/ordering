...
describe("select page model", () => {
  ...
  it("retries a failed favorites refresh through the full refresh source path after bootstrap succeeded", async () => {
    let favoriteCalls = 0;
    let categoryCalls = 0;
    ...
    await controller.runBootstrap({
      loadCategories: async () => {
        categoryCalls += 1;
        return [{ id: "cat-hot", name: "热菜", sortOrder: 0 }];
      },
      commitCategories: () => undefined,
      loadFavorites: async () => { favoriteCalls += 1; },
      loadInitialDishes: async () => ({ status: "committed" }),
    ...
    const refreshOperations = {
      ...
      run: () => controller.runDish({
        ...
        load: async () => {
          const [categories] = await Promise.all([
            ...
            (async () => {
              favoriteCalls += 1;
              if (favoriteCalls === 2) throw new Error("refresh favorites offline");
            })(),
          ]);
          return categories;
        },
      ...
      succeed: () => { refreshSuccesses += 1; },
      fail: () => { refreshFailures += 1; },
      stop: () => undefined,
    };

    const failed = await controller.runRefresh(refreshOperations);
    expect(failed.status).toBe("failed");
    expect(controller.retrySource()).toBe("refresh");
    expect({ favoriteCalls, categoryCalls, refreshFailures, recovered }).toEqual({
      favoriteCalls: 2,
      categoryCalls: 2,
      refreshFailures: 1,
      recovered: false,
    ...
    const retryFromPagePath = controller.retrySource()
      ? controller.retry()
      : Promise.resolve<SelectLoadResult>({ status: "stale" });
    const recoveredResult = await retryFromPagePath;

    expect(recoveredResult.status).toBe("committed");
    expect({ favoriteCalls, categoryCalls, refreshSuccesses, recovered }).toEqual({
      favoriteCalls: 3,
      categoryCalls: 3,
      refreshSuccesses: 1,
      recovered: true,
