...
export class FavoriteMutationCanceledError extends Error {
  constructor() {
    super("收藏操作已取消");
    this.name = "FavoriteMutationCanceledError";
  }
}
...
export function clearFavorites(): void {
  const canceledIntents = [...blockedIntents.values()];
  blockedIntents.clear();
  loadGeneration += 1;
  ...
  work.clear();
  wx.removeStorageSync(FAVORITES_STORAGE_KEY);
  const cancellation = new FavoriteMutationCanceledError();
  canceledIntents.forEach((intent) => {
    intent.waiters.forEach((waiter) => waiter.reject(cancellation));
  });
}
