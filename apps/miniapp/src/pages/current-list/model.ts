...
export type CurrentListSaveResult =
  ...
  | { status: "saved-stale"; list: SavedListDetailDto }
  | { status: "unavailable"; unavailableNames: string[] }
  | { status: "stale" }
  ...
  function run(): Promise<CurrentListSaveResult> {
    return (async () => {
      try {
        ...
        const mutationKey = draft.mutationKey;
        const response = await dependencies.fetchAvailability(draft.items.map((item) => item.dishId));
        if (dependencies.getDraft().mutationKey !== mutationKey) return { status: "stale" };

        const availability = mergeDraftAvailability(draft.items, response.items);
        dependencies.applyAvailability(availability);
        if (availability.unavailableNames.length) {
          return { status: "unavailable", unavailableNames: availability.unavailableNames };
        }

        const operation = buildSaveOperation(draft);
        ...
        if (dependencies.getDraft().mutationKey !== mutationKey) {
          return { status: "saved-stale", list };
        }
