...
import { blockFavoriteMutations, clearFavorites, settleFavoriteMutations } from "../../state/favorites";
...
Page({
  ...
  async handleOpenPrivacyContract() {
    type PrivacyApi = (options: { success(): void; fail(error: unknown): void }) => void;
  ...
  async handleClearData() {
    ...
    if (result.status === "cleared") {
      ...
      this.setData({ errorMessage: messageFor(result.error, "清空没有完成，本地内容仍然保留。") });
    }
  },
