...
import { consumeSelectEntryIntent } from "../../state/navigation";
import {
  buildDishQuery,
...
Page({
  ...
  onShow() {
    const intent = consumeSelectEntryIntent();
    if (intent === "favorites") {
      retained.filter = "favorites";
      this.setData({ filter: "favorites", refreshNotice: "" });
      if (!this.data.loading) void this.loadFavoriteItems();
      return;
    }
