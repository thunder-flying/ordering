...
function exchangeCode(code: string): Promise<StoredSession> {
  return new Promise((resolve, reject) => {
    wx.request({
      url: toApiUrl("/api/auth/wechat"),
      method: "POST",
      data: { code },
      success: (response) => {
        const body = response.data as ApiResponse<AuthSessionDto>;
        if (body.code >= 200 && body.code < 300 && body.data) {
          resolve({
...
export async function loginWithWechat(): Promise<StoredSession> {
  return exchangeCode(await wxLogin());
}
