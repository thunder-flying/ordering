...
1. Select bootstrap and dish-query ownership are separated. Category and server-favorite results commit independently even when a newer search supersedes the bootstrap dish request. Bootstrap completion refreshes the current favorites view from the latest favorite IDs, while an active all-mode search keeps ownership of its dish results.
2. Dish loading now returns an explicit `committed | stale | failed` outcome. Pull-to-refresh owns a separate monotonic token, and a newer search or category change makes the old refresh stale without showing a false network error or overwriting the newer loading state.
3. Blocking failures from bootstrap category/favorite sources retain their retry source and action. The page retry path reruns the full bootstrap sources, so the failed category and favorite calls are actually attempted again instead of retrying only cached availability work.

The orchestration controller is production-owned and is used directly by `pages/select/index.ts`; it is not a test-only helper.

### RED evidence

The replacement select suite ran before production implementation and exited with code 1:

- Select suite: 13 tests total; 9 passed and 4 failed.
- All four failures were the intended missing-feature signal: `createSelectOrchestrationController` did not exist.
- The failing behaviors covered slow bootstrap plus fast search, latest-favorites bootstrap completion, stale refresh cancellation, and full bootstrap retry after a blocking favorite/category failure.

### GREEN evidence

After the production controller and page integration were deployed:

- Select suite: 1 file, 13 tests passed.
- Full Mini Program suite: 13 files, 58 tests passed.
- TypeScript typecheck: exit 0.

### Replacement files

Fix round 2 changed two production files:

1. `apps/miniapp/src/pages/select/model.ts`
2. `apps/miniapp/src/pages/select/index.ts`

## Fix round 3/5

### Review finding addressed

After a successful bootstrap, a blocking favorites-mode pull-to-refresh failure now records the complete refresh operation as a `refresh` retry source. The status-view error button follows the same controller retry path for either bootstrap or refresh failures, so retry reruns categories, favorites, and the current dish/favorite view together. A successful retry clears the saved action; stale or user-cancelled refreshes do not leave an obsolete retry behind.

### RED evidence

The select suite was run with the new regression before production implementation:

- Command: `pnpm --filter @ordering/miniapp test -- select`.
- Result: exit 1; 13 tests passed and the one new test failed.
- The failure was precise: at `select.test.ts:317`, `retrySource()` was expected to be `refresh` but was `null` after a successful bootstrap followed by a failing refresh source.

### GREEN and review evidence

After updating the production controller and page retry path:

- Select suite: 1 file, 14 tests passed.
- Full Mini Program suite: 13 files, 59 tests passed.
- TypeScript typecheck: exit 0.
- Independent review: APPROVED; no open findings remain for the primary-pages batch.

### Replacement files

Fix round 3 changed two production files:
...
Implementation, automated GREEN gates, and independent review are complete. Completion state: **complete — review clean after fix round 3/5**.
