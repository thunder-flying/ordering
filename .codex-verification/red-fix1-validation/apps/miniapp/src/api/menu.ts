...
export function fetchDishAvailability(dishIds: string[]): Promise<{ items: DishAvailabilityDto[] }> { return request<{ items: DishAvailabilityDto[] }>({ method: "POST", path: "/api/v1/dishes/availability", data: { dishIds }, replaySafe: true }); }
