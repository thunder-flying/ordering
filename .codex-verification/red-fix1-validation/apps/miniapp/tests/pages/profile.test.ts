...
describe("profile page model", () => {
  ...
  it("settles favorite writes before remote clearing, then clears local draft and favorites only", async () => {
    ...
    expect(calls).toEqual(["confirm", "settle", "remote", "draft", "favorites"]);
  ...
  it("does not settle or clear when data clearing is canceled, and preserves local state on remote failure", async () => {
    ...
    expect(failedCalls).toEqual(["confirm", "settle", "remote"]);
    expect(failed).toEqual(localState());
  });

  it("settles favorite writes before account deletion, then clears every local identity state", async () => {
    const state = localState();
    const calls: string[] = [];
  ...
  it("does not settle or clear when deletion is canceled, and preserves all state on remote failure", async () => {
    const canceled = localState();
    const canceledCalls: string[] = [];
    await expect(runDeleteAccount({
      confirm: async (_request: DestructiveConfirmation) => { canceledCalls.push("confirm"); return false; },
