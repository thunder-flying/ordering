...
Page({
    ...
    controllers.set(this, createCurrentListSaveController({
      ...
      applyAvailability: (result) => {
        ...
        this.setData({
          ...
          availabilityLoading: false,
          availabilityError: "",
          unavailableNames: result.unavailableNames,
        });
  ...
  refreshPresentation() {
    ...
    const merged = cached
      ...
      : {
        items: draft.items.map((item) => ({ ...item, available: true })),
        unavailableNames: [] as string[],
      };
    ...
    const totalCents = sumCents(items.map((item) => (
      multiplyCents(item.referencePriceCents, item.quantity)
    )));
    const validName = characterLength(draft.name.trim()) >= 1
      && characterLength(draft.name.trim()) <= 40;
    const canSave = this.data.availabilityReady
      && validName
      && items.length > 0
      && merged.unavailableNames.length === 0
      && !this.data.saving;
    this.setData({
      name: draft.name,
      items,
  ...
  async refreshAvailability() {
    ...
    try {
      ...
      this.setData({
        availabilityReady: false,
        availabilityLoading: false,
        availabilityError: messageFor(error, "暂时无法确认菜品是否可选，请重新检查。"),
      });
      this.refreshPresentation();
    }
  },
  ...
  async handleRemove(event: WechatMiniprogram.BaseEvent) {
    ...
    const confirmed = await confirmDialog({
      title: "移出当前清单？",
      content: `将“${item.name}”移出当前清单，其他菜品会保留。`,
      confirmText: "确认移出",
      danger: true,
    });
    if (!confirmed) return;
  ...
  async handleSave() {
    ...
    if (result.status === "conflict") {
      this.setData({
        saving: false,
        conflict: true,
        saveError: messageFor(result.error, "这份清单已经更新，请重新载入后再编辑。"),
      });
    } else {
      this.setData({
        saving: false,
        saveError: messageFor(result.error, "保存没有完成，当前清单已完整保留。"),
      });
    }
    this.refreshPresentation();
  },
  ...
  async handleReloadConflict() {
    ...
    try {
      ...
      this.setData({
        saving: false,
        conflict: true,
        saveError: messageFor(error, "重新载入没有完成，当前填写内容仍然保留。"),
      });
    } finally {
      this.setData({ saving: false });
      this.refreshPresentation();
