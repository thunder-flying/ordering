...
function installWx(replies: RequestReply[]) {
  ...
  wxBoundary.request.mockImplementation((options: { success: (reply: RequestReply) => void }) => {
...
describe("authenticated request replay", () => {
  it("relogs once and replays a GET after terminal authentication", async () => {
    ...
    await expect(request<{ id: string }>({ method: "GET", path: "/api/dishes" })).resolves.toEqual({ id: "dish-1" });
    expect(wxBoundary.request).toHaveBeenCalledTimes(3);
    expect(wxBoundary.removeStorageSync).toHaveBeenCalledOnce();
  });

  it("surfaces a second 401 instead of retrying indefinitely", async () => {
    installWx([
      { code: 401, message: "session expired", data: null },
      { code: 200, message: "success", data: { token: "fresh", expiresAt: "2099-01-01T00:00:00.000Z", onboardingCompleted: true } },
    ...
    await expect(request({ method: "GET", path: "/api/dishes" })).rejects.toMatchObject({ status: 401, message: "still expired" });
    expect(wxBoundary.request).toHaveBeenCalledTimes(3);
  });
  ...
  it("refuses to replay a mutation without an idempotency key", async () => {
    ...
    await expect(request({ method: "POST", path: "/api/lists", data: { name: "今晚" } })).rejects.toMatchObject({ status: 401 });
  ...
  it("replays an idempotent mutation at most once", async () => {
    ...
    await expect(request({ method: "POST", path: "/api/lists", data: { name: "今晚" }, idempotencyKey: "idem-1" })).rejects.toMatchObject({ status: 401 });
    expect(wxBoundary.request).toHaveBeenCalledTimes(3);
  });
});

describe("request boundaries", () => {
  it("rejects an arbitrary origin and non-api path before a network call", async () => {
    installWx([]);

    await expect(request({ method: "GET", path: "https://evil.example/api/lists" })).rejects.toThrow("relative /api path");
