...
export async function loadFavorites(): Promise<void> {
  loadCached(); const response = await fetchFavorites({ limit: 50 });
  confirmed.clear(); response.items.forEach((item) => confirmed.add(item.id)); state = { ids: new Set(confirmed), loaded: true, pending: new Set() }; cache();
}
...
export function setFavorite(dishId: string, value: boolean): Promise<void> {
  loadCached(); desired.set(dishId, value); apply(dishId, value);
  const running = work.get(dishId); if (running) return running;
  const next = process(dishId); work.set(dishId, next); return next;
}
