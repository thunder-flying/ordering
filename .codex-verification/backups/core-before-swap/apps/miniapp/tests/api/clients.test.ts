...
import { fetchDishes } from "../../src/api/menu";
...
describe("typed miniapp API clients", () => {
  it("encodes menu queries", async () => {
    ...
    expect(request).toHaveBeenCalledWith(expect.objectContaining({ path: "/api/v1/dishes?q=%E8%BE%A3+%26+%E9%A6%99&categoryId=cat%2Fone&cursor=next%3F&limit=20" }));
  });

  it("uses only server-owned profile routes", async () => {
    request.mockResolvedValue(undefined); upload.mockResolvedValue(undefined);
    await updateNickname("  新名字  "); await saveAvatar("tmp/avatar.png"); await completeOnboarding();
  ...
  it("uses idempotent favorite routes", async () => {
    ...
    await fetchFavorites({ cursor: "cursor value", limit: 20 }); await addFavorite("dish/id"); await removeFavorite("dish/id");
    expect(request).toHaveBeenNthCalledWith(1, expect.objectContaining({ path: "/api/v1/favorites?cursor=cursor+value&limit=20" }));
  ...
  it("omits client-owned prices and user IDs from list write payloads", async () => {
    ...
    await copyList("list/id", { name: "复制", idempotencyKey: "00000000-0000-4000-8000-000000000001" }); await fetchLists({ cursor: "cursor value", limit: 20 });
    ...
    expect(request).toHaveBeenNthCalledWith(4, expect.objectContaining({ path: "/api/v1/lists?cursor=cursor+value&limit=20" }));
  });
});
