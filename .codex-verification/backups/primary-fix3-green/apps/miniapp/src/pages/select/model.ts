...
export function createSelectOrchestrationController(options: {
  ...
  let bootstrapRetry: (() => Promise<SelectLoadResult>) | undefined;
  ...
  const runBootstrap = async (operations: BootstrapOperations): Promise<SelectLoadResult> => {
    ...
    if (sourceError !== undefined) {
      bootstrapRetry = () => runBootstrap(operations);
  ...
  return {
    ...
    cancelRefresh,
    retrySource() {
      return bootstrapRetry ? "bootstrap" : null;
    },
    retry() {
      return bootstrapRetry ? bootstrapRetry() : Promise.resolve({ status: "stale" });
    },
    invalidate() {
      dishGeneration += 1;
      bootstrapGeneration += 1;
